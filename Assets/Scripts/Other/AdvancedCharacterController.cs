...
public class AdvancedCharacterController : MonoBehaviour
{
    ...
    [SerializeField] private float _slopRayLength = 1.5f;
    ...
    private void SlopeForce()
    {
        if (Physics.Raycast(transform.position, Vector3.down, out RaycastHit hit, _slopRayLength) == false)
            return;
